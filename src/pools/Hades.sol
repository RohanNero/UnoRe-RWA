...
error Hades__DepositTooLow(uint256 msgvalue);
...
contract Hades {
    ...
    event HadesDeposit(uint256 depositAmount, uint256 sharesReceived);
     ...
     */
    function enterPool(uint256 amount) public payable returns (uint256 sharesReceived) {
        // to ensure mUNO was sent to user we check the user's balance before deposit
        uint256 bal = IERC20(i_vault).balanceOf(msg.sender);
        ...
        uint256 shares = i_pool.deposit(amount, address(this));
        ...
        uint256 postBal = IERC20(i_vault).balanceOf(msg.sender);
