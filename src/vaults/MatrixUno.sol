 ...
 */
error MatrixUno__InvalidWeek(uint256 week);
...
contract MatrixUno is ERC4626 {
     ...
     */
    struct rewardInfo {
        uint256 rewards; // amount of STBT rewards earned by the vault during the week
        uint256 vaultAssetBalance; // total amount of assets DEPOSITED into the vault
        uint256 previousWeekBalance; // total STBT in the vault the previous week (last `performUpkeep()` call)
        uint256 currentBalance; // TOTAL AMOUNT of assets in the vault, deposited or sent from MatrixPort as rewards (balanceOf)
        uint256 claimed; // amount of STBT rewards that were claimed during the week
        uint256 deposited; // amount of STBT deposited into the vault during the week
        uint256 withdrawn; // amount of STBT withdrawn from the vault during the week
    ...
    constructor(
        ...
        rewardInfoArray.push(rewardInfo(0, 0, 0, 0, 0, 0, 0, block.timestamp, 0));
    }
     ...
     */
    function stake(uint256 amount, uint8 token, uint256 minimumPercentage)
        public
        calculateRewards
     ...
     */
    function unstake(uint256 amount, uint8 token, uint256 minimumPercentage)
        public
        calculateRewards
        returns (uint256)
    {
     ...
     */
    function claim(address addr, uint8 token, uint256 minimumPercentage) public returns (uint256, uint256) {
        ...
        for (uint256 i = lastClaimPeriod; i < currentPeriod; i++) {
            ...
            if (stakedPortion > 0) {
                uint256 userRewards = stakedPortion.mulu(rewardInfoArray[i].rewards);
                //console.log("userRewards:", userRewards);
                totalRewards += userRewards;
            }
            if (sPortion > 0) {
                uint256 userSRewards = sPortion.mulu(rewardInfoArray[i].rewards);
        ...
        if (totalRewards > 0) {
            ...
            uint256 minimumReceive = _swap(totalRewards, token, minimumPercentage);
        ...
        if (totalSRewards > 0) {
            ...
            rewardInfoArray[rewardInfoArray.length - 1].claimed += totalSRewards;
            // send stbt
    ...
    function unoClaim() public {
        ...
        rewardInfoArray[rewardInfoArray.length - 1].claimed += unaccountedRewards;
     ...
     */
    function deposit(uint256 assets, address receiver) public virtual override calculateRewards returns (uint256) {
    ...
    function performUpkeep() external {
        ...
        if (
            (currentInfo.currentBalance + currentInfo.claimed + currentInfo.withdrawn)
        ...
        rewardInfoArray.push(rewardInfo(0, currentInfo.vaultAssetBalance, currentStbt, 0, 0, 0, 0, block.timestamp, 0));
     ...
     */
    function _swap(uint256 earned, uint8 token, uint256 minimumPercentage) private returns (uint256) {
        ...
        //try
        uint256 actualReceived = pool.exchange_underlying(int128(0), int128(uint128(token + 1)), earned, minimumReceive);
     ...
     */
    function viewVaultStableBalance() public view returns (uint256 totalStableBal) {
        uint256 daiBalance = dai.balanceOf(address(this));
        uint256 usdcBalance = usdc.balanceOf(address(this));
        uint256 usdtBalance = usdt.balanceOf(address(this));
        // Add 12 zeros to USDC and USDT because they only have 6 decimals
        totalStableBal = daiBalance + (usdcBalance * 1e12) + (usdtBalance * 1e12);
    }
     ...
     */
    function viewRedeemableAt(uint256 week) public view returns (uint256 redeemable) {
     ...
     */
    function viewPortionAt(uint256 week, address addr) public view returns (int128 portion, int128 sPortion) {
        if (week >= rewardInfoArray.length) {
            revert MatrixUno__InvalidWeek(week);
        }
        uint256 daiStaked = claimInfoMap[addr].balances[0];
        uint256 usdcStaked = claimInfoMap[addr].balances[1] * 1e12;
        uint256 usdtStaked = claimInfoMap[addr].balances[2] * 1e12;
        uint256 stbtDeposited = claimInfoMap[addr].balances[3];
        uint256 totalUserStaked = daiStaked + usdcStaked + usdtStaked;
        ...
        if (totalUserStaked > 0 && unoDepositAmount > 0) {
            portion = totalUserStaked.divu(rewardInfoArray[week].vaultAssetBalance);
        } else {
            portion = 0;
        }
        if (stbtDeposited > 0) {
            sPortion = stbtDeposited.divu(rewardInfoArray[week].vaultAssetBalance);
        } else {
            sPortion = 0;
        }
     ...
     */
    function viewCurrentPeriod() public view returns (uint256) {
     ...
     */
    function viewRewards(address addr) public view returns (uint256, uint256) {
        uint256 lastClaimPeriod = claimInfoMap[addr].lastClaimPeriod;
        uint256 currentPeriod = rewardInfoArray.length - 1;
        uint256 totalRewards = 0;
        uint256 totalSRewards = 0;
     ...
     */
    function viewRewardInfo(uint256 week) public view returns (rewardInfo memory) {
     ...
     */
    function viewStakedBalance(address user, uint8 token) public view returns (uint256 balance) {
     ...
     * @param user the owner of the balance you are viewing
     */
    function viewTotalStakedBalance(address user) public view returns (uint256 totalUserStaked) {
        uint256 daiBalance = viewStakedBalance(user, 0);
        uint256 usdcBalance = viewStakedBalance(user, 1);
        uint256 usdtBalance = viewStakedBalance(user, 2);
        // Add 12 zeros to USDC and USDT because they only have 6 decimals
        totalUserStaked = daiBalance + (usdcBalance * 1e12) + (usdtBalance * 1e12);
    }
     ...
     * @param user the address that has claimed
     */
    function viewClaimedAmount(address user) public view returns (uint256) {
     ...
     */
    function viewTotalClaimed() public view returns (uint256 _totalClaimed) {
     ...
     */
    function viewTotalStaked() public view returns (uint256 _totalStaked) {
     ...
     */
    function viewUnoDeposit() public view returns (uint256) {
     ...
     */
    function viewStartingtime() public view returns (uint256) {
     ...
     */
    function viewLastUpkeepTime() public view returns (uint256) {
     ...
     */
    function viewInterval() public view returns (uint256) {
        return i_interval;
    }
    ...
    function calculateUnaccountedRewards() public view returns (uint256) {
        ...
        if (unoDepositAmount <= totalStaked) {
            ...
            uint256 remainder = unoDepositAmount - totalStaked;
            //console.log("remainder:", remainder);
            int128 portion = remainder.divu(rewardInfoArray[length - 1].currentBalance);
     ...
     */
    function viewUnaccountedRewards() public view returns (uint256) {
     ...
     */
    function viewStakeConversionRate() public view returns (int128 conversionRate) {
     ...
     */
    function viewUnstakeConversionRate() public view returns (int128 conversionRate) {
